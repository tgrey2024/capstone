...
class ScrapbookModelTest(TestCase):
    ...
    def test_scrapbook_slug_generation(self):
        ...
        scrapbook = Scrapbook.objects.create(title='Test Scrapbook', 
                                             author=self.user)
        self.assertTrue(re.match(r'^test-scrapbook-[a-f0-9]{8}$', 
                                 scrapbook.slug))
    ...
    def test_scrapbook_unique_slug(self):
        ...
        self.assertEqual(Scrapbook.objects.count(), 0)
        scrapbook1 = Scrapbook.objects.create(title='Test Scrapbook', 
                                              author=self.user)
        scrapbook2 = Scrapbook.objects.create(title='Test Scrapbook', 
                                              author=self.user)
    ...
    def test_scrapbook_ordering(self):
        ...
        scrapbook1 = Scrapbook.objects.create(title='Test Scrapbook 1', 
                                              author=self.user)
        scrapbook2 = Scrapbook.objects.create(title='Test Scrapbook 2', 
                                              author=self.user)
        scrapbook3 = Scrapbook.objects.create(title='Test Scrapbook 3', 
                                              author=self.user)
        self.assertEqual(list(Scrapbook.objects.all()), [scrapbook3, 
    ...
    def test_scrapbook_description_special_characters(self):
        ...
        scrapbook = Scrapbook.objects.create(title='Test Scrapbook', 
            author=self.user, description='Test Scrapbook! @#%^&*()_+{}:"<>?')
        self.assertEqual(scrapbook.description, 
                         'Test Scrapbook! @#%^&*()_+{}:"<>?')
    ...
    def test_scrapbook_description_max_length(self):
        ...
        description = 'a' * 500
        scrapbook = Scrapbook.objects.create(title='Test Scrapbook', 
                            author=self.user, description=description)
    ...
    def test_scrapbook_description_exceed_length(self):
        ...
        description = 'a' * 501
        scrapbook = Scrapbook(title='Test Scrapbook', author=self.user, 
...
class PostModelTest(TestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(username='testuser', 
                                             password='testpass')
        self.scrapbook = Scrapbook.objects.create(title='Test Scrapbook', 
                                                  author=self.user)

    def test_post_creation(self):
        # Test the creation of a Post instance
        self.assertEqual(self.scrapbook.posts.count(), 0)
        post = self.scrapbook.posts.create(title='Test Post', 
    ...
    def test_post_unique_slug(self):
        ...
        self.assertEqual(self.scrapbook.posts.count(), 0)
        post1 = self.scrapbook.posts.create(title='Test Post', 
                                            author=self.user, status=1)
        post2 = self.scrapbook.posts.create(title='Test Post', 
                                            author=self.user, status=1)
        self.assertNotEqual(post1.slug, post2.slug)

    def test_post_ordering(self):
        # Test the ordering of Post instances
        post1 = self.scrapbook.posts.create(title='Test Post 1', 
                                            author=self.user, status=1)
        post2 = self.scrapbook.posts.create(title='Test Post 2', 
                                            author=self.user, status=1)
        post3 = self.scrapbook.posts.create(title='Test Post 3', 
                                            author=self.user, status=1)
    ...
    def test_post_unique_slug(self):
        ...
        post1 = Post.objects.create(title='Test Post', 
                    author=self.user, scrapbook=self.scrapbook, status=1)
        post2 = Post.objects.create(title='Test Post',
                    author=self.user, scrapbook=self.scrapbook, status=1)
        self.assertNotEqual(post1.slug, post2.slug)
...
class SharedAccessModelTest(TestCase):
    ...
    def test_shared_access_creation(self):
        ...
        shared_access = SharedAccess.objects.create(user=self.user2, 
